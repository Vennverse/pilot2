...
from execution_engine import execute_step as execute_step_new, execute_plan as execute_plan_new
...
# Provider execution now delegated to execution_engine.py
# All parameters resolved and executed through registry pattern

# Wrapper around new execute_step for backward compatibility
def execute_step(step, step_results=None, user_id=None, max_retries=3, plan_id=None, step_number=0):
    ...
    for attempt in range(retries):
        try:
            if provider == "openai":
                ...
                # Handle custom user-created integrations from database
                if not user_id:
                    return False, None, "user_id required for custom integrations"
                
                # Extract provider ID (format: custom_<integration_id>)
                integration_id = provider.replace("custom_", "")
                
                try:
                    # Get integration from database
                    integration = db_manager.get_custom_integration(integration_id, user_id)
                    if not integration:
                        return False, None, f"Integration {integration_id} not found"
                    
                    # Get API key from database
                    api_key = db_manager.get_credential(user_id, integration_id, 'api_key')
                    
                    # Build request
                    base_url = integration.get("base_url") or ""
...
def manage_custom_integrations():
    ...
    if request.method == 'GET':
        try:
            ...
            # Fetch actions for each integration
...
def store_integration_credential(integration_id):
    ...
    try:
        ...
        return jsonify({"message": "Credential stored", "credential_id": str(credential['id'])}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500
